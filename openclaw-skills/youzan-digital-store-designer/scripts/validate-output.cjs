...
function hasImagePrompt(value) {
    return Boolean(cleanString(value.image_prompt)) || Object.keys(asObject(value.image_prompt_schema)).length > 0;
}
function fail(errors) {
    process.stderr.write(`Validation failed:\n${errors.map((error) => `- ${error}`).join("\n")}\n`);
    process.exit(1);
}
function collectExpectedAssetIds(schema) {
    const ids = [];
    for (const moduleValue of asArray(schema.modules)) {
        ...
        if (type === "user_assets") {
            for (const entryValue of asArray(data.entries)) {
                ...
                if (entryId)
                    ids.push(`${moduleId}.entries.${entryId}`);
            }
            continue;
        }
        if (IMAGE_MODULES.has(type)) {
            for (const itemValue of asArray(data.items)) {
                ...
                if (itemId)
                    ids.push(`${moduleId}.items.${itemId}`);
            }
        }
    }
    return ids;
...
function validateSchema(schema, requirements, errors) {
    ...
    for (const module of modules) {
        ...
        if (!id)
            errors.push("every module must have id");
        if (!SUPPORTED_MODULES.has(type))
            errors.push(`unsupported module type: ${type || "(missing)"}`);
        const data = asObject(module.data);
        if (type === "user_assets") {
            ...
            for (const entryValue of entries) {
                ...
                if (!hasImagePrompt(entry))
                    errors.push(`${id}.entries item missing image_prompt_schema`);
            }
        }
        if (IMAGE_MODULES.has(type)) {
            const items = asArray(data.items);
            if (items.length === 0)
                errors.push(`${id}.data.items must be non-empty`);
            for (const itemValue of items) {
                const item = asObject(itemValue);
                if (!cleanString(item.id))
                    errors.push(`${id}.items item missing id`);
                if (!hasImagePrompt(item))
                    errors.push(`${id}.items item missing image_prompt_schema`);
            }
        }
    }
}
function validateManifest(schema, manifest, allowMissingImages, errors) {
...
async function resolvePackageDir(rootDir) {
    if (await exists(path.join(rootDir, "shop-home-page.preview.html")))
        return rootDir;
    const distDir = path.join(rootDir, "dist");
    if (await exists(path.join(distDir, "shop-home-page.preview.html")))
        return distDir;
    return rootDir;
}
async function validatePreviewHtml(packageDir, errors) {
