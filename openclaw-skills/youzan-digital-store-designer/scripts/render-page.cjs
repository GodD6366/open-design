...
function blendSuggestion(suggestion, fallback, min, max) {
    const base = suggestion === undefined || typeof suggestion !== "number"
        ? fallback
...
function assertManifestReadyForRendering(schema, manifest, strictImages) {
    ...
    for (const moduleValue of asArray(schema.modules)) {
        ...
        if (!IMAGE_MODULE_TYPES.has(moduleType))
            continue;
...
function normalizeDesignContext(schema) {
    ...
    return {
        color_palette: {
            ...
            card_subtle: stringOr(colors.card_subtle, stringOr(theme.surface_subtle, DEFAULT_DESIGN_CONTEXT.color_palette.card_subtle)),
            text_primary: stringOr(colors.text_primary, stringOr(theme.text, DEFAULT_DESIGN_CONTEXT.color_palette.text_primary)),
            text_secondary: stringOr(colors.text_secondary, stringOr(theme.muted, DEFAULT_DESIGN_CONTEXT.color_palette.text_secondary)),
...
function fixedUserAssetsLayoutSpec(templateType) {
    if (templateType === null || templateType === USER_ASSETS_TEMPLATE_TYPES.HOTZONE)
        return null;
...
function inferTemplateTypeFromEntryCount(count) {
    if (count <= 1)
...
function resolveUserAssetsCardLayout(data) {
    ...
    if (explicitTemplateType !== null && userAssetsCardLayoutSlots(explicit).length > 0)
        return explicit;
    const legacyLayout = asObject(data.layout);
    const inputEntries = asArray(data.entries).map(asObject);
    const templateType = normalizeUserAssetsTemplateType(legacyLayout.template_type) ?? inferTemplateTypeFromEntryCount(inputEntries.length);
...
function resolveUserAssetsEntries(data, cardLayout) {
    const slots = userAssetsCardLayoutSlots(cardLayout);
    const inputEntries = asArray(data.entries).map(asObject);
    const entriesBySlotId = new Map(inputEntries
...
function resolveUserAssetsLayoutMetrics(cardLayout) {
    ...
    if (templateType === USER_ASSETS_TEMPLATE_TYPES.HOTZONE) {
        ...
        return {
            canvasWidth: 611,
            canvasHeight: rowPattern.length * USER_ASSETS_SLOT_SIZE_SPECS.free.height
...
function renderImagePlaceholder({ eyebrow, title, description, status = "待生成" }) {
...
function renderImageItem(moduleType, itemValue) {
    const item = asObject(itemValue);
...
function renderUserAssetsCard(entryValue, slotValue) {
    const entry = asObject(entryValue);
    const slot = asObject(slotValue);
...
function renderHotzoneRows(slots, entriesBySlotId) {
    const rowPattern = buildHotzoneRowPattern(slots.length || 1);
    let cursor = 0;
    return rowPattern
        .map((count) => {
...
function renderUserAssetsCardsLayout(cardLayout, entriesValue) {
    const templateType = normalizeUserAssetsTemplateType(cardLayout.template_type);
    const slots = userAssetsCardLayoutSlots(cardLayout);
    const entries = entriesValue.map(asObject);
    const entriesBySlotId = new Map(entries.map((entry) => [stringOr(entry.slot_id || entry.id), entry]).filter(([slotId]) => Boolean(slotId)));
...
function renderUserAssetsModule(module) {
    const data = asObject(module.data);
    ...
    <div class="sf-user-assets__tip">再消费 500元升为 V2</div>
...
function renderModule(moduleValue) {
    const module = asObject(moduleValue);
...
async function copyGeneratedImageBackups(rootDir, distDir, manifest) {
    const items = asObject(manifest.items);
    for (const entry of Object.values(items)) {
        const backupFile = cleanString(asObject(entry).backup_file);
        if (!backupFile || /^https?:\/\//i.test(backupFile) || path.isAbsolute(backupFile) || backupFile.includes("..")) {
            continue;
        }
        const from = path.resolve(rootDir, backupFile);
        const to = path.resolve(distDir, backupFile);
        if (!to.startsWith(`${path.resolve(distDir)}${path.sep}`)) {
            continue;
        }
        try {
            ...
            // Backups are useful for recovery, but rendering stays CDN-driven.
        }
    }
}
async function main() {
    ...
    await copyGeneratedImageBackups(rootDir, distDir, manifest);
    try {
