...
import {
  buildShopHomePageImagePrompt,
  buildShopHomePageUserAssetsEntryPrompt,
  buildStorefrontReferenceUsageNotes,
} from '@open-design/contracts/shop-home-page-image-prompts';
import {
  coerceShopHomePageStyleGuide,
  createShopHomePageSeedSchema,
  normalizeShopHomePageSchema,
} from '@open-design/contracts/shop-home-page-schema';
...
export { buildStorefrontReferenceUsageNotes };
...
/** @type {(requirements: any, styleGuide: any, options?: any) => any} */
export const createSeedSchema = (requirements, styleGuide, options = null) => {
  return /** @type {any} */ (createShopHomePageSeedSchema(requirements, styleGuide, options));
};
...
function normalizeStorefrontSchema(input, requirements, styleGuide, options = null) {
  return /** @type {any} */ (normalizeShopHomePageSchema(input, requirements, styleGuide, options));
}
  ...
  for (const module of schema.modules) {
    if (module.type === 'user_assets') {
      ...
      entries.forEach((entry, index) => {
        ...
        const referenceEntryFileName = sequentialReference ? firstEntryFileName : null;
        tasks.push({
          fileName,
          ...
          buildPromptFn: sequentialReference
            ...
            : null,
          dependsOnFileName: referenceEntryFileName,
          size:
            templateType === USER_ASSETS_TEMPLATE_TYPES.HOTZONE
              ? '1024x1024'
          ...
          buildInputImagePathsFn: (projectDir) =>
            collectUserAssetsReferenceImagePaths(entry, referenceEntryFileName, projectDir),
...
function buildImagePrompt(moduleType, item, styleGuide) {
  return buildShopHomePageImagePrompt({
    moduleType,
    item,
    styleGuide,
  });
...
function buildUserAssetsEntryPrompt(entry, slot, cardLayout, styleGuide) {
  return buildShopHomePageUserAssetsEntryPrompt({
    entry,
    slot,
    cardLayout,
    styleGuide,
...
function findStorefrontTonePresetId(value) {
  ...
  if (!needle) return '';
  const match = SHOP_HOME_PAGE_TONE_PRESETS.find(
    (preset) =>
