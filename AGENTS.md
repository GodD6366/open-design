...
- In the storefront homepage flow, the first requirements clarification must use one form with fixed core questions plus optional model-added extension questions; `module_analysis` only appears when opening reference images are available, and extension answers must persist as structured `extended_answers`.
...
- In the storefront homepage flow, each generated `user_assets` entry image defaults to one icon plus one single-line description from that entry's own subtitle; do not add the entry title, third text line, module summary text, other entry names, slogans, or extra buttons unless explicitly requested.
...
- In the storefront homepage flow, template-default reference screenshots are only initial references. The model-managed `shop-home-page.reference-state.json` decides which images are active references vs plain assets, and `shop-home-page.style-guide.json.reference_images` must mirror that effective set.
...
- In the storefront homepage flow, when a full-page reference screenshot visibly contains `user_assets`, schema generation must copy that local filename into each `user_assets.entries[*].reference_images` by default; those refs borrow the visible entry-card layout mode, icon stroke, composition, card background color, text color hierarchy, and whitespace, not the membership summary, bottom tabs, or host-app UI.
- In the storefront homepage flow, do not rely on auto-cropped module reference files. Use the full reference screenshot in `reference_images`, then constrain `top_slider` to only borrow hero atmosphere/composition and constrain `user_assets` to only borrow the visible entry-card layout/background/text-color language while generating each button subject and copy from the confirmed requirements.
...
- In the storefront homepage flow, failed asset repair via AI chat should use the same project chat plus a shell-callable `od shop-home-page-assets ...` contract; do not add a host-only chat retry path.
...
- In the storefront homepage flow, external storefront-control entrypoints must use `OD_DAEMON_URL` (or the same value passed explicitly as `--daemon-url`) as the daemon API origin. Do not derive storefront-control API calls from web ports or URLs such as `localhost:3000`, `OD_PORT`, or `OD_WEB_PORT`.
- In the storefront homepage flow, external storefront-control entrypoints must treat schema generation as a single long-running run: once confirmed requirements have been submitted, keep polling the same session/run until it reaches a terminal state and do not resubmit the same answers just because generation is slow.
...
- In the storefront homepage flow, only true external OpenClaw/storefront-control calls may auto-complete the visual clarification step. B-end manual chat must stay on the normal two-step requirements + visual clarification flow even when the project metadata still marks it as a bridge project.
...
- In the storefront homepage flow, external storefront-control / external SKILL project creation must prioritize calling `youzan-shop` to retrieve shop information (industry, shop name); when the industry is `烘焙`, automatically pick one of the three bakery templates (`bakery-doodle-toast`, `bakery-botanical-paper`, `bakery-autumn-sunroom`) at random instead of using the empty template. This behavior is scoped to those external entrypoints and is not a mandatory rule for all B-end creation flows.
