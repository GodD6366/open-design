...
- In the storefront homepage flow, full-page reference screenshots should be split into module-local crops when possible: `top_slider` uses the visible hero crop, `user_assets` uses the visible icon-strip crop plus optional per-entry crops, and unmatched customer-asset entries inherit only the strip's style cues while generating their own subject and wording from the confirmed requirements.
