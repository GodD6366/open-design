...
export function composeShopHomePageSystemPrompt({
  ...
  const parts: string[] = [
    ...
    '  "reference_regions": {',
    '    "top_slider": { "source_image": "page-shot.png", "x": 0.08, "y": 0.05, "width": 0.84, "height": 0.34 },',
    '    "user_assets": {',
    '      "strip": { "source_image": "page-shot.png", "x": 0.09, "y": 0.41, "width": 0.77, "height": 0.12 },',
    '      "entries": [',
    '        { "source_image": "page-shot.png", "x": 0.11, "y": 0.42, "width": 0.18, "height": 0.11, "target_label": "到店自取" }',
    '      ]',
    '    }',
    '  },',
    '  "analysis": {',
    ...
    '- For full-page storefront screenshots, separate style borrowing from module inference: `analysis.*` should describe reusable hero / icon / background / visible-layout cues only, and must not describe off-screen modules as confirmed facts.',
    '- When a full-page storefront screenshot visibly contains `top_slider` or `user_assets`, also write normalized `reference_regions` for those visible blocks. Coordinates must be 0..1 ratios relative to `source_image`.',
    '- `reference_regions.top_slider` is the hero-only crop. Do not use the entire page as the head image reference when you can point to the visible hero area.',
    '- `reference_regions.user_assets.strip` is the visible customer-assets icon strip. `reference_regions.user_assets.entries[]` is optional and should only be emitted for clearly isolatable visible buttons. Use `target_label` to map each entry crop back to the confirmed button title.',
    '- Use `generation_rules.must` and `generation_rules.avoid` to state the reference scope explicitly. Spell out what to borrow from the screenshot (for example background texture, icon stroke, visible hero composition) and what to ignore (for example membership bars, bottom tabs, status bars, floating widgets, or speculative next-screen content).',
    '- If the current frame visibly includes `user_assets`, make `generation_rules.must` mention that those entry cards only borrow the visible icon stroke, subject framing, title hierarchy, and whitespace rhythm from that block.',
    ...
    '- When `reference_regions.top_slider` or `reference_regions.user_assets` exists, prefer those localized crops over shared full-page screenshots when filling module `reference_images`.',
    '',
    ...
    '- `user_assets`: functional entry-card image set. Each `entries[*].image_prompt_schema` only describes its own card. Icon style should follow the page style, fixed layouts should respect the slot card size, hotzone may be freer, and the canvas background must stay plain white. Do not introduce shop logo, brand corner marks, slogan, patterns, scenery, gradients, photography, or other complex backgrounds. The entry image itself must fill its slot with straight edges: no rounded shell and no inner padding. Even when a strip or entry crop exists, the final icon subject, title, and subtitle must still follow the current button requirement instead of copying the reference crop original wording or mismatched object.',
    ...
    '- Do not copy unrelated screenshot UI into module assets. A storefront screenshot may define page mood, but it must not make the model reproduce bottom navigation, member summary strips, host-app chrome, or any unconfirmed module outside the visible frame.',
    '',
