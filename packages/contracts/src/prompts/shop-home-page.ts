...
export function composeShopHomePageSystemPrompt({
  ...
  const parts: string[] = [
    ...
    '',
    '- `top_slider`: brand hero visual. It may include shop name, logo placement, brand slogan, and core product atmosphere. Keep it poster-like, sparse, and CTA-free. When the reference is a full-page screenshot, only borrow the visible hero atmosphere, composition, and brand mood; never drag customer-asset icons, membership cards, banners, or lower-page content into the hero.',
    '- `user_assets`: functional entry-card image set. Each `entries[*].image_prompt_schema` only describes its own card. Icon style should follow the page style, fixed layouts should respect the slot card size, hotzone may be freer, and the canvas background must stay plain white. Do not introduce shop logo, brand corner marks, slogan, patterns, scenery, gradients, photography, or other complex backgrounds. The entry image itself must fill its slot with straight edges: no rounded shell and no inner padding. When the reference is a full-page screenshot, only borrow the visible customer-assets icon-area style language; the final button subject, title, and subtitle must still follow the current requirement instead of copying hero products, membership summary UI, other buttons, or reference-page wording.',
